	...
	private:
		class Node {
			public:
				...
				void update_height();
				int balance_factor();
				void balance(typename Search_tree<Type>::Node *starting_node);
				void rotateLeft(typename Search_tree<Type>::Node *starting_node);
				void rotateRight(typename Search_tree<Type>::Node *starting_node);
...
bool Search_tree<Type>::erase( Type const &obj ) {
	if ( !empty() && root_node->erase( obj, root_node ) ) {
		...
		Search_tree<Type>::Iterator the_iterator = find(obj);
		Node *remove_me = the_iterator.current_node;

		for ( Search_tree<Type>::Iterator itr = begin(); itr != end(); ++itr ) {
			if (remove_me->node_value == *itr) {
				itr.current_node->previous_node->next_node = itr.current_node->next_node;
				itr.current_node->next_node->previous_node = itr.current_node->previous_node;
				break;
			}
		}
		*/
...
void Search_tree<Type>::Node::update_height() {
	...
	tree_height = std::max(left_tree != nullptr ? left_tree->tree_height : 0, right_tree != nullptr ? right_tree->tree_height : 0);
...
bool Search_tree<Type>::Node::insert( Type const &obj, Search_tree<Type>::Node *&to_this ) {
	if ( obj < node_value ) {
		if ( left_tree == nullptr ) {
			...
			if (left_tree->node_value < backie->node_value) {
				for (Node *traverse = frontie; traverse != nullptr; traverse = traverse->next_node) {
					if (left_tree->node_value < traverse->node_value) {

						left_tree->previous_node = traverse->previous_node;
						left_tree->next_node = traverse;

						traverse->previous_node->next_node = left_tree;
						traverse->previous_node = left_tree;
				...
				left_tree->previous_node = backie;
				left_tree->next_node = backie->next_node;

				backie->next_node->previous_node = left_tree;
				backie->next_node = left_tree;
		...
		if ( right_tree == nullptr ) {
			...
			if (right_tree->node_value < backie->node_value) {
				for (Node *traverse = frontie; traverse != nullptr; traverse = traverse->next_node) {
					if (right_tree->node_value < traverse->node_value) {

						right_tree->previous_node = traverse->previous_node;
						right_tree->next_node = traverse;

						traverse->previous_node->next_node = right_tree;
						traverse->previous_node = right_tree;
				...
				right_tree->previous_node = backie;
				right_tree->next_node = backie->next_node;

				backie->next_node->previous_node = right_tree;
				backie->next_node = right_tree;
...
template <typename Type>
void Search_tree<Type>::Node::balance(typename Search_tree<Type>::Node *starting_node) {

	starting_node->update_height();


	if (starting_node->balance_factor() < -1) {
		if (starting_node->left_tree->balance_factor() > 0) {
			rotateLeft(starting_node->left_tree);
		} 
		rotateRight(starting_node);

	} else if (starting_node->balance_factor() > 1) {
		if (starting_node->right_tree->balance_factor() < 0) {
			rotateRight(starting_node->right_tree);
		}
		rotateLeft(starting_node);
	}

	if (starting_node->right_tree != nullptr) {
		starting_node->right_tree->update_height();
	}

	if (starting_node->left_tree != nullptr) {
		starting_node->left_tree->update_height();
	}
...
template <typename Type>
void Search_tree<Type>::Node::rotateRight(typename Search_tree<Type>::Node *starting_node) {

	typename Search_tree<Type>::Node *pivot = starting_node->left_tree;
	starting_node->left_tree = pivot->right_tree;
	pivot->right_tree = starting_node;
	starting_node = pivot;

}

template <typename Type>
void Search_tree<Type>::Node::rotateLeft(typename Search_tree<Type>::Node *starting_node) {

	typename Search_tree<Type>::Node *pivot = starting_node->right_tree;
	starting_node->right_tree = pivot->left_tree;
	pivot->left_tree = starting_node;
	starting_node = pivot;
